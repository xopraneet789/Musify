...
dpg.create_context()
dpg.create_viewport(title="Musify",large_icon="Musify.ico",small_icon="Musify.ico")
pygame.mixer.init()
...
def update_slider():
    ...
    while pygame.mixer.music.get_busy():
        ...
        dpg.configure_item(item="pos", default_value=pos)
        time.sleep(0.1)
    state = None
...
with dpg.window(tag="main",label="window title"):
	...
	with dpg.group(horizontal=True):
		with dpg.child_window(width=200,tag="sidebar"):
			dpg.add_text("Musify",color=(137, 142, 255))
			dpg.add_text("Built by Praneet")
			dpg.add_spacer(height=2)
